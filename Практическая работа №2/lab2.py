...
def compute_cost(X, y, theta):
    ...
    f = logistic(X @ theta)  # np.matmul()

    return (1 / m) * np.sum(-y.dot(np.log(f)) - (1 - y).dot(np.log(1 - f)))
    ...
    with open(data_file_path) as input_file:
        ...
        for line in input_file:
            *row, label = map(float, line.split(','))
            X.append([1] + row)  # фиктивный вектор + вектор признаков
            y.append(label)
...
init_theta = np.zeros((X.shape[1], 1))  # начальные значения параметров, которые мы пытаемся оптимизировать.
cost0 = compute_cost(X, y, init_theta)
print(f'Функция стоимости при начальном theta = {cost0} (должно быть ~0.693)')

# Функция стоимости, которая при наличии обучающего набора и определенного значения theta вычисляет стоимость
# логистической регрессии и градиент по отношению к theta для набора данных (X, y)

# Указание, возвращает ли функция стоимости якобиан (градиент) вместе со значением стоимостей
opt_theta_obj = minimize(lambda th: compute_cost(X, y, th),
                         init_theta,
                         method='CG',
